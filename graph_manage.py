...
class GraphManage:
    ...
    def story_scatter(self):
        if 'average_lifespan' in self.df.columns and 'average_size' in self.df.columns:
            fig = Figure(figsize=(2.5, 2))
            ax = fig.subplots()

            sns.scatterplot(data=self.df, x='average_size', y='average_lifespan', ax=ax, color='#9370DB')
    ...
    def story_bar(self):
        self.df['size_category'] = pd.Categorical(self.df['size_category'],
                                                  categories=['small', 'medium', 'big'], ordered=True)
